...
export default async function VisitPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: visit } = await supabase
    ...
    .select(`
      *,
      ...
      visit_items(*, products(id, name, unit, price, is_service, is_vaccine, booster_type, booster_interval_days, categories(is_vaccine)))
    `)
    .eq('id', id)
    .single()
  ...
  const [{ data: products }, { data: history }, { data: vaccinations }, { data: vaccines }, { data: profile }] = await Promise.all([
    supabase
      .from('products')
