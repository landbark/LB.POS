...
const dateTh = (d: string) => new Date(d).toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: '2-digit' })
...
interface VisitRow { visit_date: string; diagnosis: string | null; treatment: string | null }

export default function ReferralView({ pet, store, visits, weight }: { pet: Pet; store: StoreSettings | null; visits: VisitRow[]; weight: number | null }) {
  ...
  // ดึงประวัติรักษาล่าสุดมาเป็นค่าตั้งต้น (แก้ได้)
  const historyDefault = visits
    .filter((v) => v.diagnosis || v.treatment)
    .map((v) => `${dateTh(v.visit_date)} — ${[v.diagnosis, v.treatment].filter(Boolean).join(' / ')}`)
    .join('\n')

  const warningsDefault = [pet.allergies && `แพ้: ${pet.allergies}`, pet.chronic_conditions && `โรคประจำตัว: ${pet.chronic_conditions}`]
    .filter(Boolean).join(' · ')
    ...
    <div className="min-h-screen bg-gray-100 print:bg-white">
      ...
      <div className="flex justify-center py-6 print:py-0">
        <div className="bg-white shadow-lg print:shadow-none p-10 print:p-0" style={{ width: '720px', maxWidth: '100%', fontSize: '14px', lineHeight: 1.6 }}>
          ...
          <h1 className="text-center font-bold mb-1" style={{ fontSize: '1.25em' }}>ใบส่งตัวสัตว์ป่วย</h1>
          <p className="text-center text-gray-500 mb-5" style={{ fontSize: '0.85em' }}>Referral / Transfer Form</p>

          <p className="mb-3 text-right">วันที่ <input className={field} style={{ width: '200px' }} defaultValue={todayTh()} /></p>

          <p className="mb-3"><span className="text-gray-600">เรียน / ส่งตัวไปยัง:</span> <input className={field} style={{ width: '420px' }} placeholder="ชื่อคลินิก / โรงพยาบาลสัตว์" /></p>
          ...
          <div className="mb-3">
            <p className="mb-1"><span className="text-gray-600">อาการสำคัญ / ข้อควรระวัง:</span></p>
            <textarea rows={2} className={area} defaultValue={warningsDefault} />
          </div>

          <div className="mb-3">
            <p className="mb-1"><span className="text-gray-600">ประวัติการตรวจรักษาที่ให้ไปแล้ว:</span></p>
            <textarea rows={4} className={area} defaultValue={historyDefault} />
          </div>

          <div className="mb-3">
            <p className="mb-1"><span className="text-gray-600">ผลตรวจ / ยาที่ให้:</span></p>
            <textarea rows={3} className={area} />
          </div>

          <div className="mb-4">
            <p className="mb-1"><span className="text-gray-600">เหตุผลการส่งตัว / สิ่งที่ต้องการให้ดำเนินการต่อ:</span></p>
            <textarea rows={2} className={area} />
          </div>

          <div className="flex justify-end mt-10" style={{ fontSize: '0.9em' }}>
            <div className="text-center" style={{ width: '260px' }}>
              <div className="border-b border-gray-500 mb-1 h-6" />
              <p className="text-gray-600">ลงชื่อสัตวแพทย์ผู้ส่งตัว</p>
